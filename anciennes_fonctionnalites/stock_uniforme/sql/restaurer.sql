-- =============================================================================
-- Remettre le stock d'uniforme en service, avec ses données d'avant le retrait.
--
--     docker exec -i planif-db psql -U planif -d planif --set ON_ERROR_STOP=1 \
--         < anciennes_fonctionnalites/stock_uniforme/sql/restaurer.sql
--
-- Ce script couvre la base : tables, colonne, fonctions, vue, triggers. Deux
-- fonctions vivantes restent à compléter à la main, parce qu'elles auront
-- évolué d'ici là et qu'on ne recolle pas un morceau dans une version qu'on
-- ne connaît pas : voir la fin de fonctions.sql, et README.md pour le Python.
-- =============================================================================
...
ALTER TABLE tache ADD COLUMN IF NOT EXISTS lave_uniforme BOOLEAN NOT NULL DEFAULT FALSE;
